...
func (g *gen) expr(e ast.Expr) *vil.Value {
	...
	// A call that may fail. See throwing.go.
	case *ast.TryExpr:
		return g.tryExpr(n)

	// `&a` -- a variable handed over for the callee to write through.
	// What crosses the call is where it lives, so this is the address
	// and not the value, opened under a modify access the way SILGen
	// opens one.
	case *ast.InOutExpr:
		addr := g.lvalue(n.X)
		if addr == nil {
	...
	// A tuple is built the way SILGen builds one, from its elements
	// in order. What its bytes are is lower's question -- see
	// tupleImage, where a tuple is the struct with the same elements.
	case *ast.TupleExpr:
		// `(x)` is x in parentheses and not a one-element tuple:
		// Swift has no such thing, and the checker types it as x.
		if len(n.Elems) == 1 && n.Elems[0].Label == nil {
			return g.expr(n.Elems[0].X)
		}
		elems := make([]*vil.Value, 0, len(n.Elems))
	...
	case *ast.SelfExpr:
		// In an initializer self is storage rather than a value that
		// was handed over, so reading it is a load.
...
// consume is rvalue with the value already in hand: what makes a
// value one whose ownership is being handed over. A borrowed one is
// copied, and an owned one is no longer this scope's to destroy.
func (g *gen) consume(v *vil.Value) *vil.Value {
	if v == nil {
		return nil
	}
...
func (g *gen) literal(e *ast.BasicLit) *vil.Value {
	// `nil` is a case of an optional rather than a value of its own,
	// and which optional is whatever it is being written into. The
	// checker decided that already; this builds the case.
	if e.Kind == token.NIL {
...
func (g *gen) constant(e ast.Expr) *vil.Value {
	...
	t := g.typeOf(e)
	// A literal written where an optional is wanted is that optional's
	// case with the literal inside it. The constant itself is of the
	// payload's type -- `let a: Int32? = 7` is a four-byte seven, not
	// a seven of whatever width the optional is -- so it is built
	// there and wrapped after. See optional.go.
	payload := t
	...
	case analyzer.IntValue:
		...
		// `0` in `return 0` from a function returning one is the
		// floating-point zero, not an integer that something later
		// converts.
		if isFloat(payload) {
			bits := int64(math.Float64bits(float64(v.Int)))
			if b, ok := payload.Underlying().(*types.Basic); ok && b.Kind() == types.Float {
				bits = int64(math.Float32bits(float32(v.Int)))
			}
			made = g.blk.Struct(lowerType(payload),
				g.blk.FloatLiteral(vil.Object(builtinFor(payload)), bits))
			break
		}
		raw := g.blk.IntegerLiteral(vil.Object(builtinFor(payload)), int64(v.Int))
		made = g.blk.Struct(lowerType(payload), raw)
	case analyzer.FloatValue:
		// The bits, because that is what a float literal is by the
		// time it reaches a register. Which width they are is the
		// type's: a Float is four bytes and rounds here, once, rather
		// than again somewhere below.
		bits := int64(math.Float64bits(v.Float))
		if b, ok := payload.Underlying().(*types.Basic); ok && b.Kind() == types.Float {
			bits = int64(math.Float32bits(float32(v.Float)))
		}
		raw := g.blk.FloatLiteral(vil.Object(builtinFor(payload)), bits)
		made = g.blk.Struct(lowerType(payload), raw)
...
func (g *gen) member(e *ast.MemberExpr) *vil.Value {
	...
	// `Chrono.mean` with no call after it: a function named through
	// its module, used as a value. The checker resolved it to the
	// same symbol the bare spelling reaches, so this is the same
	// function value that spelling would produce.
	if fs, ok := g.info.Uses[e.Name].(*analyzer.FuncSymbol); ok {
		return g.funcValue(fs)
	}
	// A dot whose left side is a module and whose right side is not a
	// function is something this does not have a value for -- a type,
	// or a global. Left alone it would lower to nothing and say
	// nothing, which is how a statement disappears out of a program
	// that still compiles.
	...
	// A stored property of the type rather than of an instance: its
	// storage is reached through an accessor, not at an offset.
	if recv, f, ok := g.staticProperty(e); ok {
		return g.staticRead(e, recv, f)
	}
	// A property of a String or an Array, which is a call into the
	// standard library: neither has a field this compiler may look
	// at. See string.go.
...
func (g *gen) call(e *ast.CallExpr) *vil.Value {
	...
	if mem, ok := e.Fun.(*ast.MemberExpr); ok {
		// Unless the dot is a module's rather than a value's:
		// `Chrono.epochYearUTC()` names a free function the long way,
		// and the checker resolved it to the same symbol the short
		// spelling would reach. What is on the left is not a
		// receiver, and calling it as a method would evaluate a
		// module.
		...
		// `Shape.line(42)` looks like a call and is a case being
		// made: the arguments are what it carries. See payloadCase.
		if ec, ok := g.info.Uses[mem.Name].(*analyzer.EnumCaseSymbol); ok {
			return g.payloadCase(e, ec)
		}
		return g.method(e, mem)
	}
	// `.line(7)` -- a case named the short way and called, which is
	// the case carrying what the parentheses hold.
...
// callFunc lowers a call to a function named by a declaration, however
// the call spelled the name.
func (g *gen) callFunc(e *ast.CallExpr, sym *analyzer.FuncSymbol) *vil.Value {
	return g.callFuncTry(e, sym, false)
}

// callFuncTry is callFunc, with `optional` set where the call is
// written `try?` and its failure becomes nothing rather than a value.
func (g *gen) callFuncTry(e *ast.CallExpr, sym *analyzer.FuncSymbol, optional bool) *vil.Value {
	// A call to a function whose result is an existential. One
	// declared here was refused where it was written -- see
	// functionNamed -- so there is no body to call and nothing to say
	// that has not been said. Reporting it again here would give one
	// mistake two diagnostics and send the reader looking for two.
	//
	// An imported one has a body: the library returns five words
	// through storage this call sets aside, and reading them is
	// agreeing with a layout rather than producing one.
	if _, isEx := existentialOf(sym.Signature().Results); isEx {
		if _, imported := g.info.Imported[sym]; !imported {
			return nil
		}
	}
	// A generic function has no body until something says what its
	// type parameters are. This call did; it gets a copy of the body
	// made for those types. See generic.go.
	if len(sym.Signature().TypeParams) > 0 {
		...
		// One this module did not declare has no body to specialize.
		// It is called as it stands, with the metadata that says what
		// the types are. See imported_generic.go.
		if _, imported := g.info.Imported[sym]; imported {
	...
	if sym.Signature().Throws {
		return g.tryApply(e, ref, args, sym.Signature().Results, optional)
	}
...
func (g *gen) construct(e *ast.CallExpr, tn *analyzer.TypeNameSymbol) *vil.Value {
	...
	// answer, so a call goes to the initializer it declared.
...
func (g *gen) makeClass(e *ast.CallExpr, tn *analyzer.TypeNameSymbol, cl *types.Class, t types.Type) *vil.Value {
	// A class that says how it is made is made that way, whether the
	// initializer's body is here or in another module. What differs
	// is the receiver: a class declared elsewhere is allocated by
	// reading its size out of the metadata, and one declared here by
	// this compiler's own alloc_ref. See applyInit.
	if len(cl.Inits) > 0 {
...
// payloadCase builds a case that carries a value: `Shape.line(42)`.
//
// It is written like a call and is not one -- what the parentheses
// hold is what the case carries. A case carrying several values
// carries a tuple of them, which is what the checker recorded as its
// associated type and what lowering expects to place.
func (g *gen) payloadCase(e *ast.CallExpr, ec *analyzer.EnumCaseSymbol) *vil.Value {
	...
	var payload *vil.Value
	switch {
	case len(args) == 1:
		payload = g.rvalue(args[0].X)
	default:
		vals := make([]*vil.Value, 0, len(args))
		for _, a := range args {
			v := g.rvalue(a.X)
			if v == nil {
				return nil
...
func (g *gen) method(e *ast.CallExpr, mem *ast.MemberExpr) *vil.Value {
	...
	// A method of the type rather than of an instance. `Vec.zero()`
	// has no receiver to pass -- what stands in for one is the
	// metatype, and a struct's is thin, which is no register at all.
	// See staticCall.
	if ref.Method.IsStatic {
		return g.staticCall(e, ref, recv)
	}
	// The checker resolved the call against whatever it knew, and
	// inside a generic body that is a protocol's requirement rather
	// than any implementation of it. Here the type argument is known,
	// so the question is asked again against what it became: this is
	// where `a.value()` on a `T: Describable` becomes a call to the
	// value() that T's type argument actually declares.
	if resolved, ok := g.witness(ref, recv); ok {
		ref = resolved
	}
	// A value whose type arrives with it: the implementation is found
	// through the table it carries rather than named here. See
	// witness.go.
...
//
// The arguments themselves come from arguments(), which puts them in
// parameter order and fills in the defaults a call left out.
func (g *gen) methodArgs(e *ast.CallExpr, sig *types.Signature, receiver func() *vil.Value) (*vil.Value, []*vil.Value, bool) {
	self := receiver()
	if self == nil {
		g.unsupported(e)
...
func (g *gen) methodSymbol(ref *analyzer.MethodRef) string {
	...
	d.Context = nominalChain(ref.Recv)
...
// nominalChain is the types a declaration is nested inside,
// outermost first.
//
// A method on `Chart.Point` is nested in both, and its symbol says
// both: naming only the innermost gives `main.Point.sum()` where
// swiftc defined `Charts2.Chart.Point.sum()`, which is a symbol
// nothing has.
...
// declareSignature states a declaration's type from a signature.
//
// Apart from declare so that an instantiation can state its own: a
// specialization's type is the generic one with the arguments
// substituted, which is not what its symbol's signature says.
func (g *gen) declareSignature(f *vil.Func, sig *types.Signature) {
	for _, p := range sig.Params {
		t := lowerType(p.BodyType())
		conv := paramConvention(p, t)
		// One passed by address is declared as one. What crosses the
		// call is a pointer, and a declaration that named the value
		// instead described a parameter of forty bytes in a register
		// -- which is not a convention, it is a type with no machine
		// form. The definition side already says this; see gen.go's
		// parameter loop, which is the same decision from the other
		// end.
		if byAddress(conv) {
			t = t.Address()
		}
	...
	// A function that may fail says so, because that is what puts the
	// failure in the error register rather than in the return
	// sequence. What it fails with is `any Error`, whose conformance
	// this compiler names its own way -- so the type here is the
	// reference it is at run time, and nothing reads through it.
	if sig.Throws {
		f.SetThrows(vil.Object(vil.BuiltinNativeObj))
	}
}
...
// operate applies an operator to two values that are already lowered,
// and answers nothing where core knows no implementation for it.
//
// It is the half of binary that a compound assignment shares: `x += y`
// is `x = x + y`, and the plus in the middle is this.
func (g *gen) operate(at ast.Node, op string, operand, results types.Type, lhs, rhs *vil.Value) *vil.Value {
	// An operator whose implementation is a call rather than an
	// instruction. Every operator on String is one. See string.go.
...
// staticCall lowers a call to a method of the type: `Vec.zero()`.
//
// It is an ordinary call whose symbol ends in Z and whose receiver is
// the metatype rather than a value. A struct's metatype is thin -- the
// type is known, so there is nothing to carry -- which is why nothing
// is passed for it and why the parameter list is the declared one.
func (g *gen) staticCall(e *ast.CallExpr, ref *analyzer.MethodRef, recv types.Type) *vil.Value {
	// `Vec.zero()` reads the name off the type, so what the receiver
	// expression has is the metatype. The symbol is the type's.
	if meta, ok := recv.(*types.Metatype); ok {
		recv = meta.Instance
	}
	if _, isClass := recv.Underlying().(*types.Class); isClass {
		// A class's static method takes the metatype as its receiver
		// and that metatype is a real pointer, which is the same
		// machinery an allocating initializer needs. Refused until
		// this passes one.
		g.refuse(e, "a static method of a class, whose receiver is the type's metadata")
		return nil
	}
	callee := g.m.Func(g.staticSymbol(ref, recv)).SetSourceName(ref.Method.Name)
	if g.needsType(callee) {
...
// staticSymbol names a method of the type. It is the method symbol
// with Z after it, which is what Swift writes and what mangle already
// knows how to say.

...
func (p *printer) operands(in *vil.Inst) {
	...
	case vil.AllocStack, vil.AllocRef:
		// A slot standing for a binding says so: `alloc_stack $T, let,
		// name "v"`. One holding a temporary has neither, and its
		// attributes read as flags rather than as a declaration.
		if aux.Name != "" {
			p.printf(" %s", aux.Type)

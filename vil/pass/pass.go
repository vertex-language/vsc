...
func Mandatory(m *vil.Module) error {
	...
	for _, f := range m.Funcs() {
		// The marks go first: they are notes to definite
		// initialization, and every pass after this should see the
		// storage rather than the note. Promotion in particular looks
		// at what uses a box, and a mark is not one of the uses it
		// knows.
		eraseMarks(f)
		promoteBoxes(f)
		resolveAssigns(f)

...
type WitnessTable struct {
	...
	Linkage  Linkage
	Entries  []TableEntry
	// Symbol is the object-file name of the table, which is swiftc's:
	// `$s3App4MineV3Lib8MeasuredAAWP`. Mangling is the front end's, so
	// it is decided where the conformance is read and carried here.
	Symbol string
	// Descriptor is the symbol of the record that says what the table
	// is a conformance of, which is the same name with Mc for WP.
	Descriptor string
	// ProtocolSymbol is the protocol descriptor the conformance
	// points at: `$ss5ErrorMp` for Error. Empty for a protocol this
	// module declared, which has no descriptor yet.

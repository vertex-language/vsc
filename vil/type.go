...
func trivial(t types.Type) bool {
	...
	case *types.Enum:
		...
		return true
	// An optional owns what it is holding and nothing else. `Int32?`
	// owns nothing, so it needs no retain and no release; `Box?` owns
	// a Box when it has one. Saying nothing here made every optional
	// non-trivial, and a release was emitted for a value that is four
	// bytes and a tag.
	case *types.Optional:

...
func (o Op) Consumes(i int) bool {
	switch o {
	// The forwarding instructions: what they produce stands in for
	// what they were given, so the operand is given away. Marking is
	// one of them when it marks a box -- SILGen marks an
	// initializer's self that way, destroys the mark's result, and
	// never mentions the box again.
	case DestroyValue, Return, Throw, MoveValue, EndLifetime,
		MarkUninitialized:
		return i == 0
	// Taking a tuple apart is giving it away: what comes out stands
	// in for what went in, so the tuple itself is gone and each
	// element owns its own share.
	case DestructureTuple:
		return i == 0
	// An existential is storage rather than a register, so what ends
	// its life is destroy_addr -- which consumes it for the same
	// reason destroy_value consumes what it is given.
	case DestroyAddr:
		return i == 0
	case Store, Assign:

...
func lower(t *testing.T, src string) *sil.Module {
	t.Helper()
...
// next one: what it emits is sound by the rules sil/verify holds.
...
func program(t *testing.T, path string) (src, fn string) {
	...
	name, ok := strings.CutPrefix(trimmed, "// sil:")
	if !ok {
		name, ok = strings.CutPrefix(trimmed, "// vil:")
	}
	if !ok {
		t.Fatalf("%s: the first line must name the function to compare, as `// sil: name`", path)
	}
...
// the instructions.
func normText(t *testing.T, m *sil.Module, name string) string {
	t.Helper()
	return text.Normalize(funcText(t, m, name))
}

func funcText(t *testing.T, m *sil.Module, name string) string {
	t.Helper()

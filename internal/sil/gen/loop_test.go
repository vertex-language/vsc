...
// TestConditionTemporariesEndBeforeTheBranch: a loop condition that reads
// a copy of an array ends that copy in the header, before the branch into
// the body -- not once, after the loop.
//
// The copy is a retain of the array's storage. Held while the body writes
// an element, it makes the storage look shared, and every write copies the
// whole array first: `while i < a.count { a[i] = x }` becomes quadratic,
// and a loop over a few megabytes of pixels never finishes.
func TestConditionTemporariesEndBeforeTheBranch(t *testing.T) {
	got, diags := generate(t, "main", `
func main() -> Int32 {
	...
	// The header runs from its label to the branch that leaves it.
	start := strings.Index(got, "bb1:")
	if start < 0 {
		t.Fatalf("no loop header:\n%s", got)
	}
	end := strings.Index(got[start:], "cond_br")
	if end < 0 {
	...
	if !strings.Contains(header, "destroy_value") && !strings.Contains(header, "end_borrow") {
		t.Errorf("the copy made to count the array is still alive at the branch:\n%s", header)
	}
}

...
func TestEntryPoint(t *testing.T) {
	for _, c := range []struct {
		...
		// An async main is run as the first task: the entry hands it to
		// the runtime's executor and exits zero when it has finished.
		// Its type says @async, as Swift's does -- an async function has
		// a calling convention of its own, so the type carries it.
		{"async main runs as the first task", `func main() async {}`, []string{
			"sil hidden [ossa] @$vsc_async_main : $@convention(thin) @async () -> () {",
			"sil [ossa] @main : $@convention(c) () -> Int32 {",
			"%2 = function_ref @vertex_async_main",
			"%3 = apply %2(%1)",
			"return %5",
		}},

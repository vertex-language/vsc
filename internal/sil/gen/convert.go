...
// whether it was one.
func (g *gen) convert(e *ast.CallExpr, to types.Type) (*sil.Value, bool) {
	var args []*ast.CallArg
	if e.Args != nil {
		args = e.Args.Args
	}
	// `Int32(truncatingIfNeeded: x)` keeps the low bits of x, extended
	// by its sign where the destination is wider, and checks nothing.
	if len(args) == 1 && args[0].Label != nil && g.text(args[0].Label) == "truncatingIfNeeded" {
	...
	if src.min < dst.min {
		lo := g.blk.IntegerLiteral(sil.Object(builtinFor(from)), dst.min)
	...
	if src.max > dst.max {
		hi := g.blk.IntegerLiteral(sil.Object(builtinFor(from)), int64(dst.max))
...
func (g *gen) compareRaw(op string, a, b *sil.Value, t types.Type) *sil.Value {
	bi, ok := core.Lower(op, t)
	if !ok {
		return nil
	}
	return g.blk.Builtin(bi.Name, sil.Object(builtinNamed(bi.Result)), a, b)
}
...
// floatConvert lowers conversions to Double and Float.
func (g *gen) floatConvert(e *ast.CallExpr, arg ast.Expr, from, to types.Type) (*sil.Value, bool) {
	v := g.rvalue(arg)
	if v == nil {
		return nil, true
	}
	...
	if isFloat(from) {
		...
		out := g.blk.Builtin(verb+"_"+src+"_"+dst, sil.Object(builtinNamed(dst)), raw)
...
// intFromFloat lowers float-to-integer conversion with bounds checking against destination range.
func (g *gen) intFromFloat(e *ast.CallExpr, arg ast.Expr, from, to types.Type,
	dst intRange) (*sil.Value, bool) {
	...
	src := floatBuiltinName(from)
	srcT := sil.Object(builtinNamed(src))
	...
	for _, check := range []struct {
		...
		bad := g.blk.Builtin("xor_Int1", sil.Object(sil.BuiltinInt1),
			ok, g.blk.IntegerLiteral(sil.Object(sil.BuiltinInt1), -1))
	...
	out := g.blk.Builtin(verb+"_"+src+"_"+dst.machine,
		sil.Object(builtinNamed(dst.machine)), raw)
	return g.blk.Struct(lowerType(to), out), true
}
...
// truncatingConvert lowers `T(truncatingIfNeeded: x)` between integer
// types: the same bits, cut to T's width or extended by x's sign.
func (g *gen) truncatingConvert(e *ast.CallExpr, arg ast.Expr, to types.Type) (*sil.Value, bool) {
	from := g.typeOf(arg)
	dst, ok := intRangeOf(to)
	if !ok {
		return nil, false
	...
	if src.machine != dst.machine {
		...
		out = g.blk.Builtin(verb+"_"+src.machine+"_"+dst.machine, sil.Object(builtinFor(to)), out)
	}
	return g.blk.Struct(lowerType(to), out), true
}

...
// TestAPromotedBoxDestroysWhatItHolds: releasing a box destroyed its
// value, and deallocating a stack slot does not, so a var holding
// something owned has it taken out and destroyed before the slot goes.
// swiftc -emit-sil writes destroy_addr then dealloc_stack for this.
func TestAPromotedBoxDestroysWhatItHolds(t *testing.T) {
	got := mandatory(t, `
func names() -> Int {
    var xs = ["a"]
    xs = ["b", "c"]
...
`)
	take := strings.LastIndex(got, "load [take]")
	destroy := strings.LastIndex(got, "destroy_value")
	dealloc := strings.LastIndex(got, "dealloc_stack")
	if take < 0 || destroy < take || dealloc < destroy {
		t.Errorf("the array in the slot is not destroyed before the slot is deallocated:\n%s", got)
	}
}

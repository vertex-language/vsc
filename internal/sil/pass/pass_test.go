...
func TestConventionsSurvive(t *testing.T) {
	...
	if got := f.Entry().Args()[0].Ownership(); got != sil.None {
...
func TestStageMoves(t *testing.T) {
	...
	bb := f.Entry()
	bb.Return(bb.Tuple(sil.Object(types.Typ[types.Void])))

	if err := Mandatory(m); err != nil {
		t.Fatal(err)
	}
	if m.Stage() != sil.StageCanonical {
...
func TestMandatoryRejects(t *testing.T) {
	...
	if m.Stage() != sil.StageRaw {
		t.Errorf("a module that was refused moved to %s", m.Stage())
	}
}

// TestCorpusLowers takes every program sil/gen is held to, runs it
...
func lowerFile(t *testing.T, path string) *sil.Module {
	t.Helper()

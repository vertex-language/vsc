...
func TestBorrowedParameter(t *testing.T) {
	...
	m := sil.NewModule("a", sil.StageRaw)
	f := m.Func("borrows").SetLinkage(sil.Hidden).SetAttr("ossa")
	b := f.Param(sil.Object(box), sil.ParamGuaranteed)
	f.SetResult(sil.Object(types.Typ[types.Int]), sil.ResultUnowned)

	bb := f.Entry()
	n := bb.RefElementAddr(b, "Box.n", sil.Object(types.Typ[types.Int]))
	v := bb.Load(n, "trivial")
	bb.Return(v)
...
func TestSwitchEnum(t *testing.T) {
	...
	m := sil.NewModule("a", sil.StageRaw)
	f := m.Func("matches").SetLinkage(sil.Hidden).SetAttr("ossa")
	...
	zero := dot.IntegerLiteral(sil.Object(sil.BuiltinIntLiteral), 0)
	dot.Br(join, zero)

	k := line.Arg(intT, sil.None)
	line.Br(join, k)
...
func TestOwnership(t *testing.T) {
	box := types.NewNamed("Box", "", &types.Class{Name: "Box"})
	m := sil.NewModule("a", sil.StageRaw)
	f := m.Func("owns").SetLinkage(sil.Hidden).SetAttr("ossa")
	b := f.Param(sil.Object(box), sil.ParamOwned)
	f.SetResult(sil.Object(box), sil.ResultOwned)
...
func funcText(t *testing.T, f *sil.Func) string {
	t.Helper()

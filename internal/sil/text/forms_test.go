...
func TestInstructionForms(t *testing.T) {
	intT := sil.Object(types.Typ[types.Int])
	boolT := sil.Object(types.Typ[types.Bool])
	...
	shapeT := sil.Object(shape)

	cases := []struct {
		name string
		emit func(b *sil.Block, args []*sil.Value)
		want []string
	}{
		{"ownership", func(b *sil.Block, a []*sil.Value) {
			c := b.CopyValue(a[0])
			bo := b.BeginBorrow(c)
			b.EndBorrow(bo)
		...
		{"stack memory", func(b *sil.Block, a []*sil.Value) {
			s := b.AllocStack(intT)
		...
		{"a box is a var", func(b *sil.Block, a []*sil.Value) {
		...
		{"access scopes", func(b *sil.Block, a []*sil.Value) {
		...
		{"enums", func(b *sil.Block, a []*sil.Value) {
		...
		{"literals and metatypes", func(b *sil.Block, a []*sil.Value) {
			b.IntegerLiteral(sil.Object(sil.BuiltinInt64), 42)
			b.IntegerLiteral(sil.Object(sil.BuiltinInt1), -1)
		...
		{"builtins", func(b *sil.Block, a []*sil.Value) {
			raw := b.StructExtract(a[1], "Int._value", sil.Object(sil.BuiltinInt64))
			bit := b.Builtin("cmp_slt_Int64", sil.Object(sil.BuiltinInt1), raw, raw)
		...
		{"references and calls", func(b *sil.Block, a []*sil.Value) {
			b.AllocRef(boxT)
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			m := sil.NewModule("t", sil.StageRaw)
			m.Func("callee")
			f := m.Func("f").SetAttr("ossa")
			b := f.Param(boxT, sil.ParamGuaranteed)
			n := f.Param(intT, sil.ParamUnowned)
			flag := f.Param(boolT, sil.ParamUnowned)
			_ = flag

			bb := f.Entry()
			c.emit(bb, []*sil.Value{b, n, flag})
			bb.Unreachable()
...
func TestTerminatorForms(t *testing.T) {
	...
	s := f.Param(sil.Object(shape), sil.ParamUnowned)
	cond := f.Param(sil.Object(sil.BuiltinInt1), sil.ParamUnowned)
	f.SetResult(intT, sil.ResultUnowned)

	entry := f.Entry()
	dot, line, join, dead := f.Block(), f.Block(), f.Block(), f.Block()
	entry.SwitchEnum(s,
		sil.Case{Member: "Shape.dot!enumelt", Dest: dot},
		sil.Case{Member: "Shape.line!enumelt", Dest: line})
	dot.CondBr(cond, join, []*sil.Value{dot.IntegerLiteral(intT, 1)},
		dead, nil)
	line.Br(join, line.IntegerLiteral(intT, 2))
	out := join.Arg(intT, sil.None)
	join.Return(out)
	dead.Unreachable()

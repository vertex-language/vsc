...
// builds the same function in SIL, prints it, normalizes both sides,
// and requires them to agree. Once sil/gen exists this is what it
...
//
//   - symbols, because SIL does not clone Swift's mangling
...
// TestMatchesSwiftSIL builds `func borrows(_ b: Box) -> Int` in SIL
...
func TestMatchesSwiftSIL(t *testing.T) {
	...
	if w := Normalize(string(want)); got != w {
		t.Errorf("ours and swiftc disagree.\n--- ours\n%s\n--- swiftc\n%s", got, w)
	}
...
func TestOwnedParameter(t *testing.T) {
	...
	box := types.NewNamed("Box", "", &types.Class{Name: "Box"})
	m := sil.NewModule("a", sil.StageRaw)
	f := m.Func("consumes").SetLinkage(sil.Hidden).SetAttr("ossa")
	...
	if copied.Ownership() != sil.Owned {
		t.Errorf("copy_value produced %v, want @owned", copied.Ownership())
	}
	if borrow.Ownership() != sil.Guaranteed {

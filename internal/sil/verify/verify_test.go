...
var (
	boxT    = sil.Object(classType("Box"))
	intT    = sil.Object(types.Typ[types.Int])
	optIntT = sil.Object(&types.Optional{Wrapped: types.Typ[types.Int]})
)

// fn starts a function in ownership form.
func fn(name string) *sil.Func {
	m := sil.NewModule("t", sil.StageRaw)
	return m.Func(name).SetLinkage(sil.Hidden).SetAttr("ossa")
}

// wants runs the verifier and requires exactly the named fault.
func wants(t *testing.T, f *sil.Func, sentinel error) {
	t.Helper()
	err := Func(f)
	if err == nil {
...
func clean(t *testing.T, f *sil.Func) {
	t.Helper()
...
func TestOwnedParameterVerifies(t *testing.T) {
	f := fn("consumes")
	b := f.Param(boxT, sil.ParamOwned)
	f.SetResult(boxT, sil.ResultOwned)
...
func TestLeak(t *testing.T) {
	f := fn("leak")
	f.Param(boxT, sil.ParamOwned) // never consumed
	f.SetResult(intT, sil.ResultUnowned)
	bb := f.Entry()

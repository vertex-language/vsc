...
func TestTypePredicates(t *testing.T) {
	...
	// An array is compared element by element, so it is comparable where
	// its elements are.
	if !Comparable(&Array{Elem: Typ[Int]}) {
		t.Errorf("[Int] should be comparable")
	}
	if Comparable(&Array{Elem: &Struct{Name: "Opaque"}}) {
		t.Errorf("an array of a struct that is not comparable should not be")
	}
}

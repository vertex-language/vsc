...
var Typ = [...]*Basic{
	...
	Float16:       {kind: Float16, info: IsFloat, name: "Float16"},
	BFloat16:      {kind: BFloat16, info: IsFloat, name: "BFloat16"},
}
...
// vertexNames is how Vertex writes each universe name that Swift writes
// differently, where there is one to choose.
//
// It is not the reverse of vertexTypes, because that map is many-to-one:
// `float` and `float32` both name Float, and `double` and `float64` both
// name Double. The widths are chosen here for the same reason int32 is
// spelled int32 -- a name that says how wide it is beats one that does
// not. Every entry is checked against vertexTypes by the tests, so the
// two cannot drift.
var vertexNames = map[string]string{
	"Bool":      "bool",
	"Int":       "int",
	"Int8":      "int8",
	"Int16":     "int16",
...
// VertexName is how Vertex spells a universe type that Swift spells
// differently, and false for a name Vertex writes the same way.
//
// The two denote one type. It is for writing Vertex -- a module
// interface is Vertex source -- and never for a diagnostic, which names
// a type the way the language being compiled does.
func VertexName(swift string) (string, bool) {
	name, ok := vertexNames[swift]
	return name, ok
}

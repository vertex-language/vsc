...
// indirect reports whether a value of this type is passed and
// returned by address rather than in registers, and how many bytes it
// takes if so.
//
// Swift's rule, and the same number on both sides of a call: a struct
// of more than four words goes by address. A parameter is then a
// pointer to it -- swiftc's own code for a five-word argument reads
// the fields out of x0 rather than out of x0 to x4 -- and a result is
// written into storage the caller set aside, whose address it passes
// in the register AAPCS64 reserves for one. That is sret, and it is
// the reason a value of such a type never lives in registers here at
// all: there are not enough.
func indirect(t vil.Type) (size int64, yes bool) {
	// An existential is five words and never travels in registers:
	// swiftc's own `makeOne` writes the buffer, the metadata and the
	// table through x8, which is sret. It is not a struct, so it does
	// not reach the size rule below by itself.
	...
	// Only a struct this package can lay out. One it cannot is
	// refused where it is used, with a message about the layout
	// rather than about the ABI.
	if _, ok := structLeaves(st); !ok {
		return 0, false
	}
	// The size decides, not whether some other path could place it.
	// directWords says no to a struct of one field as well -- that
	// one is its field and machineOf forwards to it -- so asking
	// "not direct" would send a wrapper around an Int32 through
	// memory.
...
func existentialBytes(t vil.Type) (int64, bool) {
	if !t.IsValid() || t.IsAddress() || t.Formal() == nil {
		return 0, false
	}
	switch f := t.Formal().(type) {
	case *types.Existential:
		// `Any` is four words rather than five: a buffer and the
		// metadata, with no table because there is nothing to look up
		// in one.
...
// outResult reports whether an instruction's result comes back
// through storage the caller set aside, and how many bytes it needs.
//
// Two things say so, and they are the same thing at different scales.
// A value too wide for registers has nowhere else to go, which is
// what indirect answers. And a call whose result convention is `@out`
// returns that way whatever its size: SIL's `@out` is a statement
// about the convention rather than about the type, and a generic
// function returns its result indirectly because the caller is the
// only one who knows how big it is. Array's subscript getter is
// `<T> (Int, @guaranteed Array<T>) -> @out T`, and swiftc's own call
// to it for an [Int32] passes a four-byte slot in x8.
func outResult(in *vil.Inst, v *vil.Value) (int64, bool) {
	if n, yes := indirect(v.Type()); yes {
		return n, true
	}
	switch in.Op() {
	case vil.Apply, vil.TryApply:
	default:
		return 0, false
	}
	args := in.Args()
	...
	ft, _ := args[0].Type().Formal().(*vil.FuncType)
	if ft == nil || len(ft.Results) == 0 || ft.Results[0].Convention != vil.ResultOut {
		return 0, false
	}
...
func storageBytes(t vil.Type) (int64, bool) {
...
func storageAlign(t vil.Type) int64 {
	if !t.IsValid() || t.Formal() == nil {
		return 8
	}
...
func indirectAlign(t vil.Type) int64 {
...
// tupleParts is the registers a tuple parameter travels in, one per
// element, and reports whether it is a tuple that travels that way.
//
// A tuple parameter is not a struct parameter. swiftc's own code for
// `sumTuple(_ t: (Int32, Int32))` reads w0 and w1 -- the elements,
// each in its own register -- while `plus(_ a: Vec, _ b: Vec)` reads
// x0 and x1, each holding a whole Vec packed into a word. A tuple
// parameter is the parameter list, and Swift flattens it into one.
//
// Only a tuple whose elements are each one register. An element that
// is a struct would be packed into words of its own, which is a
// second rule this does not need yet and would be wrong to guess at.
func tupleParts(t vil.Type) ([]repr, bool) {
	if !t.IsValid() || t.IsAddress() || t.Formal() == nil {
		return nil, false
	}

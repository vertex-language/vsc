...
// TestAWideStructGoesByAddress: past four words Swift passes and
// returns one by address, so this package does too. The parameter is
// a pointer and the result is storage the caller set aside, whose
// address is the sret parameter -- which is what swiftc's own code
// for the same declaration does.
func TestAWideStructGoesByAddress(t *testing.T) {
	m, err := lowerSrc(t, `
struct TooWide {
    var a: Int; var b: Int; var c: Int; var d: Int; var e: Int
}
func first(_ w: TooWide) -> Int { return w.a }
func make() -> TooWide { return TooWide(a: 1, b: 2, c: 3, d: 4, e: 5) }
`)
	if err != nil {
		t.Fatalf("a wide struct was refused: %v", err)
	}
	got := dump(t, m)
	// The parameter is the address of the struct, not its words.
	if !strings.Contains(got, "firstySiAA7TooWideVF(%a0 ptr) i64") {
		t.Errorf("a wide parameter is not a pointer:\n%s", got)
	}
	// The result is written through the caller's storage, so the
	// function takes that address and returns nothing -- and says so
	// in its signature, which is what puts the address in X8 rather
	// than in X0. Saying it only at the call would agree with itself
	// and with nothing else.
	if !strings.Contains(got, "makeAA7TooWideVyF(%a0 ptr sret ") {
		t.Errorf("a wide result does not come back through sret:\n%s", got)
	}
}

...
@fatal0:
  %9 = ptr.getaddr @$vsc_fatal_636afad1def18851
  call @vertex_fatal(%9)
  trap

@cont2:
  %10 = call @$s1t3fibyS2iF(%7)
  %11 = i64.add %5, %10
  %12 = i64.saddo %5, %10
  brif %12, @fatal0, @cont3

@cont3:
  return %11
}

internal global ro @$vsc_fatal_636afad1def18851 [20]i8 align 1 = "arithmetic overflow"

import func @vertex_fatal(ptr) nounwind
`
	if body(got) != want {
		t.Errorf("got:\n%s\nwant:\n%s", body(got), want)
	}
...
func TestRefusesTheOwnershipForm(t *testing.T) {
	...
	m = sil.NewModule("t", sil.StageCanonical)
	if _, err := Module(m, target, Options{}); !errors.Is(err, ErrStage) {
...
//
// The example is a struct with a Bool? in it. A Bool? is a type this
// package has no representation for, and a struct holding one has no
// layout -- so the parameter cannot be passed, and saying so is the
...
// TestCorpusLowers takes the programs sil/gen is held to and requires
...
func TestCorpusLowers(t *testing.T) {
	...
	files, err := filepath.Glob("../internal/sil/gen/testdata/*.swift")
...
// lowered takes a source file all the way to a SIL module this
// package will accept.
func lowered(t *testing.T, path string) *sil.Module {
	t.Helper()

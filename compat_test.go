...
// link. main.swift's are the program now, the way Swift runs them; in
// any other file of a module they still have nowhere to go, and are
// reported.
func TestTopLevelCodeReported(t *testing.T) {
	helpers := vsc.Source{Name: "helpers.swift", Text: []byte(`
func side(_ a: int32) -> int32 { return a }

side(3)
`)}
	program := vsc.Source{Name: "main.swift", Text: []byte(`
func main() -> int32 { return side(0) }
`)}
	_, diags := vsc.Compile([]vsc.Source{helpers, program}, vsc.Options{Module: "main", Stop: vsc.Lowered})
	if !vsc.Errors(diags) {
		t.Fatal("top-level code outside main.swift was accepted and dropped")
	}
	if !strings.Contains(diags[0].Message, "top-level code runs only in main.swift") {
		t.Errorf("diagnostic does not name the problem: %s", diags[0])
	}
}

// TestTopLevelCodeIsTheProgram: in main.swift, or in a program of one
// file, the statements are the entry point, and a function named main
// beside them is an ordinary function.
func TestTopLevelCodeIsTheProgram(t *testing.T) {
	src := vsc.Source{Name: "main.swift", Text: []byte(`
func main() -> int32 { return 3 }

let n = main()
print(n)
`)}
	if _, diags := vsc.Compile([]vsc.Source{src}, vsc.Options{Module: "main", Stop: vsc.Lowered}); vsc.Errors(diags) {
		for _, d := range diags {
			t.Errorf("%s", d)
		}
	}
...
func TestTwoModulesSharingAName(t *testing.T) {
	iface := func(name string) vsc.Source {
		return vsc.Source{Name: name + ".vinterface", Text: []byte(
...
// TestStaticStoredProperty is the other half: a stored static needs
// storage and a one-time initializer, and it has them -- the addressor
// a read of it calls, filling its storage the first time.
func TestStaticStoredProperty(t *testing.T) {
	const src = `
...
// left a class reference nothing destroyed -- invalid SIL rather than

...
// Contains reports whether a Pos is one of this file's.
//
// A Pos is an offset into one file's source and carries no mark of
// which file that was, so this is a range check and can say yes to a
// position that came from a file of the same size. It is what keeps a
// printer from asking for a line that is not there.
func (f *File) Contains(p Pos) bool {
	return f != nil && p.IsValid() && int(p) <= len(f.src)+1
}

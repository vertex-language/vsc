...
// standardProtocols are the standard library's protocols the scheme
// gives a letter of their own, after an S as the types' are: SQ is
// Equatable, and so on. swiftc writes $sSQMp for Equatable's descriptor.
var standardProtocols = map[string]byte{
	"Equatable":        'Q',
	"Comparable":       'L',
	"Hashable":         'H',
	"Sequence":         'T',
...
func (m *mangler) stdProtocol(module, name string) bool {
	...
	code, ok := standardProtocols[name]
	if !ok {
		return false
	}
	m.endRun()
...
// the nominal type alone: Array is Sa, whatever its element.
func (m *mangler) extendedType(t types.Type) error {
	switch t := t.(type) {
	case *types.Basic:
		return m.basic(t)
	case *types.Array:
...
func (m *mangler) typ(t types.Type) error {
	...
	// Dictionary and Set are standard substitutions too, taking their
	// arguments as Array does.
	case *types.Dictionary:
		m.write("SD")
		m.writeByte('y')
		if err := m.typ(t.Key); err != nil {
			return err
		}
		if err := m.typ(t.Value); err != nil {
			return err
		}
		m.writeByte('G')
		return nil

...
func TestFunction(t *testing.T) {
	...
	for _, c := range []struct {
		...
		// Async before throws, as swiftc writes them.
		{"async",
			Decl{Module: "n", Name: "g11", Signature: &types.Signature{Async: true}},
			"$s1n3g11yyYaF"},
		{"async throwing",
			Decl{Module: "n", Name: "g12", Signature: &types.Signature{Async: true, Throws: true}},
			"$s1n3g12yyYaKF"},
	} {
...
// whole symbol.
// TestOperatorNames holds operators to swiftc's spelling: each character
// a letter, then oi for infix or op for prefix. The strings are swiftc's,
// from `nm` over the same declarations in a module m.
func TestOperatorNames(t *testing.T) {
	intT := types.Typ[types.Int]
	boolT := types.Typ[types.Bool]
	doubleT := types.Typ[types.Double]
	for _, c := range []struct {
		name string
		decl Decl
		want string
		...
		{"== (Int, Bool) -> Bool",
			Decl{Module: "m", Name: "==", Signature: sig([]*types.Param{p("", intT), p("", boolT)}, boolT)},
			"$s1m2eeoiySbSi_SbtF"},
		{"&+ (Bool, Int) -> Int",
			Decl{Module: "m", Name: "&+", Signature: sig([]*types.Param{p("", boolT), p("", intT)}, intT)},
			"$s1m2apoiySiSb_SitF"},
		{"prefix ! (Double) -> Bool",
			Decl{Module: "m", Name: "!", Signature: sig([]*types.Param{p("", doubleT)}, boolT)},
			"$s1m1nopySbSdF"},
		{"<=> (Int, Int) -> Int",
			Decl{Module: "m", Name: "<=>", Signature: sig([]*types.Param{p("", intT), p("", intT)}, intT)},
			"$s1m3legoiyS2i_SitF"},
	} {
		got, err := Function(c.decl)
		if err != nil {
...
// TestStandardProtocols holds conformances to the standard library's
// protocols to swiftc's symbols: the ones with a letter of their own are
// written with it. From `nm` over struct A in module m conforming to each.
func TestStandardProtocols(t *testing.T) {
	a := Decl{Module: "m", Context: []Nominal{{Name: "A", Kind: Struct}}}
	for _, c := range []struct{ proto, descriptor, conformance string }{
		{"Equatable", "$sSQMp", "$s1m1AVSQAAMc"},
		{"Comparable", "$sSLMp", "$s1m1AVSLAAMc"},
		{"Hashable", "$sSHMp", "$s1m1AVSHAAMc"},
		{"Sequence", "$sSTMp", "$s1m1AVSTAAMc"},
		{"IteratorProtocol", "$sStMp", "$s1m1AVStAAMc"},
		{"CustomStringConvertible", "$ss23CustomStringConvertibleMp", "$s1m1AVs23CustomStringConvertibleAAMc"},
		{"Error", "$ss5ErrorMp", "$s1m1AVs5ErrorAAMc"},
	} {
		d, err := ProtocolDescriptor("Swift", c.proto)
		if err != nil || d != c.descriptor {
			t.Errorf("%s descriptor: got %s (%v), want %s", c.proto, d, err, c.descriptor)
		}
		got, err := ConformanceDescriptor(Conf{Type: a, ProtocolModule: "Swift", ProtocolName: c.proto, Module: "m"})
		if err != nil || got != c.conformance {
			t.Errorf("%s conformance: got %s (%v), want %s", c.proto, got, err, c.conformance)
		}
	}
}

// TestExtensionsOfStandardTypes holds what an extension in module main
// adds to Int, String, Dictionary and Optional to swiftc's names for it.
func TestExtensionsOfStandardTypes(t *testing.T) {
	...
	for _, c := range []struct {
		...
		{"a method of Int",
			func() (string, error) {
				return Function(Decl{Module: "main", Extended: intT, Name: "times",
					Signature: sig([]*types.Param{p("_", fnT)}, nil)})
			}, "$sSi4mainE5timesyyyyXEF"},
		{"a mutating one is no different",
			func() (string, error) {
				return Function(Decl{Module: "main", Extended: intT, Name: "bump", Signature: sig(nil, nil)})
			}, "$sSi4mainE4bumpyyF"},
		{"a static one",
			func() (string, error) {
				return Function(Decl{Module: "main", Extended: intT, Name: "parse", Static: true,
					Signature: sig([]*types.Param{p("_", strT)}, intT)})
			}, "$sSi4mainE5parseySiSSFZ"},
		{"a getter of Int",
			func() (string, error) {
				return Getter(Decl{Module: "main", Extended: intT, Name: "doubled", Signature: sig(nil, intT)})
			}, "$sSi4mainE7doubledSivg"},
		{"a getter of String",
			func() (string, error) {
				return Getter(Decl{Module: "main", Extended: strT, Name: "shout", Signature: sig(nil, strT)})
			}, "$sSS4mainE5shoutSSvg"},
		{"of Dictionary",
			func() (string, error) {
				return Getter(Decl{Module: "main", Extended: &types.Dictionary{Key: strT, Value: intT}, Name: "n", Signature: sig(nil, intT)})
			}, "$sSD4mainE1nSivg"},
		{"of Optional",
			func() (string, error) {
				return Getter(Decl{Module: "main", Extended: &types.Optional{Wrapped: intT}, Name: "isSome", Signature: sig(nil, boolT)})
			}, "$sSq4mainE6isSomeSbvg"},
	} {
		got, err := c.got()
...
// TestConformancesOfStandardTypes: what module main says of Int and String.
func TestConformancesOfStandardTypes(t *testing.T) {
	for _, c := range []struct {
		typ        types.Type
		table, rec string
	}{
		{types.Typ[types.Int], "$sSi4main5NamedAAWP", "$sSi4main5NamedAAMc"},
		{types.Typ[types.String], "$sSS4main5NamedAAWP", "$sSS4main5NamedAAMc"},
	} {
		conf := Conf{Type: Decl{Module: "main", Extended: c.typ}, ProtocolModule: "main", ProtocolName: "Named", Module: "main"}
		table, err1 := WitnessTable(conf)
		rec, err2 := ConformanceDescriptor(conf)
		if err1 != nil || err2 != nil || table != c.table || rec != c.rec {
			t.Errorf("%s: got %q %q (%v %v), want %q %q", c.typ, table, rec, err1, err2, c.table, c.rec)
		}
	}
}

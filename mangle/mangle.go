...
type Decl struct {
	...
	// nothing.
	Context []Nominal
	// Extended is, for a declaration an extension adds to a type of the
	// standard library -- Int, String, Array -- that type. The symbol
	// names the type and then the module the extension is in, with E
	// after it: `$sSi4mainE5timesyyyyXEF` is main's Int.times.
	Extended types.Type
...
// NominalType returns the base mangled symbol for a nominal type without suffixes
// (e.g. `$s4main4MineV` for struct Mine in module main).
...
func conformance(c Conf, suffix string) (string, error) {
	...
	m.write("$s")
	if c.Type.Extended != nil {
		// A standard library type is written as itself: the conformance,
		// not the type, is what another module declared.
		if err := m.extendedType(c.Type.Extended); err != nil {
			return "", err
		}
	} else if err := m.context(c.Type); err != nil {
		return "", err
	}
	if !m.stdProtocol(c.ProtocolModule, c.ProtocolName) {
...
// MetadataAccessor returns the mangled symbol of the function that returns a type's metadata (suffix `Ma`).
...
// Getter returns the mangled symbol for a computed property getter (suffix `vg`).
func Getter(d Decl) (string, error) { return accessor(d, "vg") }

// StaticGetter returns the mangled symbol for a static computed property getter (suffix `vgZ`).
func StaticGetter(d Decl) (string, error) { return accessor(d, "vgZ") }

// Setter returns the mangled symbol for a computed property setter (suffix `vs`).
...
//
// Initializer returns the mangled symbol for an allocating initializer (suffix `cfC`).
...
// mangler maintains state for emitting Swift ABI mangled symbol strings,
// including identifier word compression and type substitution tables.
type mangler struct {
...
func (m *mangler) context(d Decl) error {
	if d.Extended != nil {
		if err := m.extendedType(d.Extended); err != nil {
			return err
		}
...
// operatorLetters are the letters swiftc spells each operator character
// with in a symbol.
var operatorLetters = map[byte]byte{
	'&': 'a', '@': 'c', '/': 'd', '=': 'e', '>': 'g', '<': 'l', '*': 'm',
	'!': 'n', '|': 'o', '+': 'p', '?': 'q', '%': 'r', '-': 's', '~': 't',
	'^': 'x', '.': 'z',
}

// operatorName is an operator's name as a symbol spells it, and whether
// the name is an operator at all.
func operatorName(name string) (string, bool) {
	if name == "" {
		return "", false
	}
...
func (m *mangler) signature(sig *types.Signature) error {
	...
	// Async before throws, as swiftc writes them: `f() async throws` is
	// `...YaKF`.
	if sig.Async {
		m.write("Ya")
	}
...
// Discriminator returns a file-unique discriminator string for private declarations.

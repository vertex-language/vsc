...
func runInteropCase(t *testing.T, swiftc string, target ir.Target, dir string) {
	...
	rt, err := build.RuntimeForSwift(target)
	if err != nil {
		t.Fatal(err)
	}
	...
	bridge, err := build.SwiftBridge(target)
	if err != nil {
		t.Fatal(err)
	}
	bridgeObj := filepath.Join(work, "bridge.o")
	if err := os.WriteFile(bridgeObj, bridge.Data, 0o644); err != nil {
		t.Fatal(err)
	}
	...
	link := exec.Command(swiftc, append([]string{"-o", bin, appObj, rtObj, bridgeObj}, libObjs...)...)

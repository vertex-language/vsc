...
//
// The interface does not say which of the two a library was built
// for. swiftc emits the same declarations either way -- the text
// differs only in the header -- so a client cannot tell a resilient
// library from a fixed-layout one by reading its interface, only by
// reading the `-enable-library-evolution` in its `swift-module-flags`
// line. Reading that line and refusing what follows from it is the
// missing guard: a program built against a resilient library today
// compiles, links, and dies on the first field read.
//

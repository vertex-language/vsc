...
func (c *checker) checkFuncBody(d *ast.FuncDecl, scope *Scope) {
	...
	for _, p := range sig.Params {
		// A parameter is a constant, which is what makes `n = 1` in a
		// body an error -- except an inout one, which is the caller's
		// storage handed over for the callee to write through, and
		// writing to it is the whole point of it.

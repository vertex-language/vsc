...
type Info struct {
	...
	// KernelDescriptors is each call the checker wrote for a launch's
	// kernel -- gpu._kernelDescriptor() in the chain `k.Launch(...)` is
	// rewritten to -- by the kernel it names. See kernel.go.
	KernelDescriptors map[*ast.CallExpr]*FuncSymbol

	// KernelMaps is, for a descriptor call written for `k.Map(...)`, which
	// of the element kernel's parameters are mapped over buffers: bit i
	// for parameter i. The descriptor is the grid kernel the compiler
	// writes for that choice. See kernel.go.
	KernelMaps map[*ast.CallExpr]uint64

	// Kernels is every function this module declares `kernel`, in the
...
func NewInfo() *Info {
	return &Info{
		...
		KernelDescriptors: make(map[*ast.CallExpr]*FuncSymbol),
		KernelMaps:        make(map[*ast.CallExpr]uint64),

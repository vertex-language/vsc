...
type Info struct {
	...
	//
	// Generics are monomorphised -- one copy of the body per set of
	// type arguments -- so lowering needs the arguments themselves
	// rather than the substituted signature, since it has to walk the
	// body substituting as it goes. The signature is what the rest of
	// the check saw and is recorded against the call in Types.
	Specializations map[*ast.CallExpr]Specialization

	// Defaults is the expression a parameter's default is, for the
	// parameters that have one.
	//
	// It is here rather than on types.Param because types/ holds no
	// expressions, and it is needed because Swift evaluates a default
	// at the call: a call that leaves a parameter out has to supply
	// the value itself, and what value that is lives on the
	// declaration -- which may be in another module's interface.
...
// A Specialization is one instantiation of a generic declaration:
// what each of its type parameters was inferred to be.
//
// Args is in the order the declaration wrote its parameters, which is
// the order a name for the instantiation is built from -- two calls
// that infer the same types are one instantiation, and the order is
// what makes that comparable.

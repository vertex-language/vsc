...
func (pg *PrecedenceGraph) initBuiltins() {
	...
	// Builtin operator mappings. These are the groups the standard
	// library declares them in, and an operator missing from here does
	// not merely lose a nicety: OperatorGroup falls back to
	// ComparisonPrecedence, which is non-associative, so `a & b == 8`
	// folded as `a & (b == 8)` rather than `(a & b) == 8`.

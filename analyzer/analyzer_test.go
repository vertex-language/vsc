...
`
	...
	// 6. A let given its value on each branch is initialized after them
	srcBranches := `
let d: Int
if true {
    d = 1
...
let readD = d + 1
`
	fileBranches, fBranches := parseSnippet(t, srcBranches)
	_, diagsBranches := Check([]*ast.File{fileBranches})
	for _, d := range diagsBranches {
		t.Errorf("unexpected diag on branch initialization: %s", d.Print(fBranches))
	}

	// 7. Given its value twice on one branch
	srcTwice := `
let e: Int
if true {
    e = 1
...
`
	fileTwice, _ := parseSnippet(t, srcTwice)
	if _, diagsTwice := Check([]*ast.File{fileTwice}); len(diagsTwice) == 0 {
		t.Fatalf("expected error on second let assignment within a branch, got none")
	}
}
...
// TestGenericConstraintsAreChecked: a type argument has to satisfy
// what its parameter promised.
//
// Nothing checked before this, so a type with none of the
// requirements could be passed to a constrained generic and the
// mistake surfaced -- if at all -- as a call to a method the type
// does not have, inside a specialization, a long way from the call
// that caused it.
//
// Every verdict and every message below is swiftc's, on the same
// source.
func TestGenericConstraintsAreChecked(t *testing.T) {
	const decls = `
protocol Valued { func value() -> Int32 }
protocol Named: Valued { func tag() -> Int32 }

struct Yes: Valued { func value() -> Int32 { return 1 } }
struct Full: Named {
    func value() -> Int32 { return 1 }
    func tag() -> Int32 { return 2 }
}
struct No { func other() -> Int32 { return 1 } }

func take<T: Valued>(_ x: T) -> Int32 { return x.value() }
func named<T: Named>(_ x: T) -> Int32 { return x.tag() }
func free<T>(_ x: T) -> Int32 { return 0 }
`
	cases := []struct {
		name string
		body string
		...
		{"a conforming type is accepted",
			"func f() -> Int32 { return take(Yes()) }", ""},
		{"a type that does not conform is not",
			"func f() -> Int32 { return take(No()) }",
			"requires that 'No' conform to 'Valued'"},
		{"conformance through an inherited protocol counts",
			"func f() -> Int32 { return take(Full()) }", ""},
		{"but not the other way round",
			"func f() -> Int32 { return named(Yes()) }",
			"requires that 'Yes' conform to 'Named'"},
		{"an unconstrained parameter takes anything",
			"func f() -> Int32 { return free(No()) }", ""},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := checkSnippet(t, decls+tc.body)
			if tc.want == "" {
				if len(got) != 0 {
					t.Errorf("reported %v, want a clean check", got)
				}
...
// TestConstrainedExtensionMembers: what an extension with a where clause
// adds to a generic type is there only for the instances whose arguments
// meet it, and inside it the clause holds.
func TestConstrainedExtensionMembers(t *testing.T) {
	const decls = `
struct Box<T> { var v: T }
extension Box where T: Equatable {
    func same(_ o: Box<T>) -> Bool { return v == o.v }
    var twice: Bool { return v == v }
}
extension Box { var plain: Int { return 1 } }
struct NoEq { var n: Int }
`
	cases := []struct {
		name string
		body string
		...
		{"an instance whose argument conforms has the members",
			"func f() -> Bool { return Box(v: 1).same(Box(v: 2)) && Box(v: \"a\").twice }", ""},
		{"one whose argument does not is refused the method",
			"func f() -> Bool { return Box(v: NoEq(n: 1)).same(Box(v: NoEq(n: 1))) }",
			"referencing instance method 'same' on 'Box' requires that 'NoEq' conform to 'Equatable'"},
		{"and the property",
			"func f() -> Bool { return Box(v: NoEq(n: 1)).twice }",
			"referencing property 'twice' on 'Box' requires that 'NoEq' conform to 'Equatable'"},
		{"but not what an unconstrained extension adds",
			"func f() -> Int { return Box(v: NoEq(n: 1)).plain }", ""},
		{"and the clause does not leak out of its extension",
			"func g<T>(_ b: Box<T>) -> Bool { return b.v == b.v }", "is not comparable"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := checkSnippet(t, decls+tc.body)
...
// TestAClassInitializerGivesItsArgumentsContext: a class is made by an
// initializer it declares -- Swift gives the memberwise one to structs
// alone -- so the call has to be checked against the one it names.
//
// Nothing did, so every argument was read with no context. `Counter(n:
// 41)` passed an Int where the initializer takes an Int32, and the
// mistake surfaced far away, in lowering, as an argument whose width
// did not match the signature it was going to.
func TestAClassInitializerGivesItsArgumentsContext(t *testing.T) {
	for _, c := range []struct{ name, src, want string }{
		{"a literal takes the parameter's type", `
final class Counter {
    var n: Int32
    init(n: Int32) { self.n = n }
}
func use() -> Int32 { return Counter(n: 41).n }
`, ""},
		{"a wrong label is reported", `
final class Counter {
    var n: Int32
    init(n: Int32) { self.n = n }
}
func use() -> Int32 { return Counter(k: 41).n }
`, "incorrect argument label (have 'k:', expected 'n:')"},
		{"a wrong type is reported", `
final class Counter {
    var n: Int32
    init(n: Int32) { self.n = n }
}
func use() -> Int32 { return Counter(n: true).n }
`, "cannot convert"},
	} {
		t.Run(c.name, func(t *testing.T) {
			msgs := checkSnippet(t, c.src)
...
// TestModuleVariablesIgnoreFileOrder: a variable declared at module scope
// in one file is in scope in another, whichever of the two was read first.
// Swift has no ordering rule for a declaration at that scope, and this
// checker's types and functions never had one either.
//
// A file with top-level code in it keeps its order, because there a
// variable is initialized where it is written; that is covered by the
// definite-initialization tests above.
func TestModuleVariablesIgnoreFileOrder(t *testing.T) {
	user, uf := parseSnippet(t, `
func useIt() -> Int { return capacity + counter }
`)
	decls, _ := parseSnippet(t, `
let capacity = 64
var counter: Int = 0
`)
	for _, order := range [][]*ast.File{{user, decls}, {decls, user}} {
		_, diags := Check(order)
		for _, d := range diags {
			t.Errorf("unexpected diag: %s", d.Print(uf))
		}
	}
}

// TestReceiverMethodNamedLikeAType: a receiver method called Size, on a
// type in a package that also declares a type Size, neither hides the type
// in the signatures of other methods nor turns `Size(...)` in its own body
// into a call of itself. A receiver method has no implicit self, so the
// type's members are not in its unqualified scope; its type names are.
func TestReceiverMethodNamedLikeAType(t *testing.T) {
	file, f := parseSnippet(t, `
public struct Size {
    public var Width: Float
    public init(_ width: Float) { Width = width }
}
public struct Window { public let Id: Int32 }
public func (w: borrowing Window) Size() -> Size {
    return Size(Float(w.Id))
}
public func (w: borrowing Window) SetSize(_ size: Size) {}
`)
	info, diags := Check([]*ast.File{file})
	for _, d := range diags {
		t.Errorf("unexpected diag: %s", d.Print(f))
	}
	ast.Inspect(file, func(n ast.Node) bool {
		...
		if id, ok := call.Fun.(*ast.IdentExpr); ok && id.Name.Text(f) == "Size" {
			if _, isType := info.Uses[id.Name].(*TypeNameSymbol); !isType {
				t.Errorf("Size(...) in the method resolves to %T, want the type", info.Uses[id.Name])
			}
		}
		return true
	})

...
func (p *parser) tryEnumCasePattern(mode patternMode) ast.Pattern {
	...
	// A qualified type: `tcp.TcpError.timedOut(_)` names the case of a
	// type another module declares, and every step but the last is the
	// type's. The grammar's TypeIdentifier is dotted; only the final
	// member is the case.
	for p.at(token.PERIOD) {
		...
		t = &ast.MemberType{Span: p.span(lo), X: t, Dot: dot, Name: name}
	}

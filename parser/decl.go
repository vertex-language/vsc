...
// parseDecl parses a top-level or member declaration.
...
// parseReceiver parses a method receiver clause `(name: [mod] Type)`.
func (p *parser) parseReceiver() *ast.Receiver {
	...
	// Parse optional receiver ownership modifier (borrowing, consuming, inout).
...
// parseOwnership consumes an ownership modifier keyword if present.
...
func (p *parser) parseImport(lo token.Pos, attrs []*ast.Attr, mods []*ast.Modifier) ast.Decl {
	...
	// Parse folder-path imports (string or parenthesized string list).
	switch {
...
func (p *parser) parseVarDecl(lo token.Pos, attrs []*ast.Attr, mods []*ast.Modifier) ast.Decl {
	...
	for {
		...
		// Attach accessor block or implicit getter body if on same line or introducing accessors.
...
// atAccessorBlock reports whether the upcoming brace begins an accessor block rather than a code block.
...
// accessorWordAt reports whether token n ahead is an accessor introducer (get, set, willSet, didSet, _read, _modify, etc.).
...
// parseExecKind parses an optional execution modifier (`kernel` or `graph`).
...
func (p *parser) parseDeinit(lo token.Pos, attrs []*ast.Attr, mods []*ast.Modifier) ast.Decl {
	...
	p.next()
	// Deinitializer body is optional (e.g. in protocols or module interfaces).
	if p.at(token.LBRACE) {
		d.Body = p.parseCodeBlock()
	}
	d.Span = p.span(lo)
...
// parseEnumCase parses an enum case declaration with optional associated values and raw values.
...
// checkRawValue verifies that x is a valid literal raw value.
...
// parseIfConfigMembers parses a conditional compilation block inside a type member block.
...
// checkMember verifies whether declaration d is permitted in container c.

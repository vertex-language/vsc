...
func TestCompile(t *testing.T) {
	...
	if len(u.Files) != 1 || u.Info == nil || u.SIL == nil {
		t.Error("a phase left nothing behind")
	}
	if got := u.SIL.Stage(); got != sil.StageLowered {
...
func TestStop(t *testing.T) {
	for _, c := range []struct {
		...
		{vsc.Raw, sil.StageRaw, true, false},
		{vsc.Canonical, sil.StageCanonical, true, false},
		{vsc.Lowered, sil.StageLowered, true, false},
		{vsc.All, sil.StageLowered, true, true},
	} {
		t.Run(string(c.stage)+"/"+itoa(int(c.stop)), func(t *testing.T) {
			...
			if (u.SIL != nil) != c.sil {
				t.Errorf("ownership IR present = %v, want %v", u.SIL != nil, c.sil)
			}
			if (u.VIR != nil) != c.vir {
				t.Errorf("machine IR present = %v, want %v", u.VIR != nil, c.vir)
			}
			if c.sil && u.SIL.Stage() != c.stage {
				t.Errorf("stage is %s, want %s", u.SIL.Stage(), c.stage)
			}
		})
	}
}
